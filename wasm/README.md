...
pass the module as `opts.grid_renderer`. (The simple `<pre>` DOM renderer
survives as a first-class module — `wasm/web/src/neovim-ui-pre.ts`, UMD
`neovim-ui-pre.js`, shipped in the bundles for canvas-free embedding.) The full renderer design
...
`.github/workflows/wasm.yml` does this automatically on every push. The expensive
wasm compile + site assembly is factored into a shared composite action
(`.github/actions/build-wasm`) and runs **once** in a `build` job; its artifacts
then fan out to every downstream job: `test` (the engine/JS suites + an upstream
functional-suite sample), `test-tvim-e2e` (headless-Chrome browser→engine→server),
`deploy` (uploads the site to Pages, push builds only) and `build-tvim`
(cross-compiles the `tvim` Go server — see "As a standalone application" — for
linux/darwin amd64+arm64 with that bundle baked in via `-tags embed_assets`, and
uploads each binary as a `tvim-<os>-<arch>` artifact). The assembled demo site is
itself downloadable as the `nvim-wasm-demo` artifact.
