...
reloading. The simple `<pre>` DOM renderer is kept as a first-class module
(`web/src/neovim-ui-pre.ts` → UMD `web/dist/neovim-ui-pre.js`, shipped in
the site/lib bundles for canvas-free embedding).

## Build scripts

| Script | Produces / does | Prereqs · key flags |
|---|---|---|
| `web/build-ts.sh` | Compiles the page + library **TypeScript** (`web/src/*.ts`) into `web/dist/` (gitignored): `neovim{,-ui,-ui-pre,-utils}.js` (UMD + global), `.mjs` (ESM), `.d.ts`/`.d.mts`, `app.js`, `engine-worker.js`. Three `tsc` passes (no bundler); cores are UMD-wrapped by `web/tools/umd-wrap.mjs`. Also runnable as `npm run build` in `wasm/web`. | Needs `npm install` in `wasm/web` (the `typescript` devDependency). |
...
| `build-deps.sh` | Cross-compiles libuv, PUC Lua 5.1, lpeg, luv, tree-sitter (+ parsers), unibilium, utf8proc → `.deps-wasm/usr`. Everything is built **`-fPIC`** (the engine links `-sMAIN_MODULE=2` for dynamic tree-sitter grammars; non-PIC archives fail the nvim link with `R_WASM_MEMORY_ADDR` relocation errors). Vendors libuv internal headers into the install (for `uv_stubs.c`). | **Fast path**: exits early if `.deps-wasm/usr/lib/liblua.a` exists **and** the `.built-with-fpic` marker is present — a pre-PIC install is rebuilt automatically. CI caches only `.deps-wasm/usr` (never the configure tree — it embeds emsdk's absolute path; the cache key hashes this script). Force a rebuild by removing `.deps-wasm`. |
...
| `tvim/download-tvim.sh [os] [arch]` | Pulls the latest **CI-built** `tvim-<os>-<arch>` artifact (from the `wasm` workflow) → `./tvim`. Defaults to host os/arch. | Needs `gh`. Env: `REPO` (default `TyOverby/neovim`), `BRANCH` (default `main`), `OUT` (default `.`). |
| `chrome-ext/build-ext.sh [out=_ext]` | Assembles the **unpacked Chrome extension** ("edit any `<textarea>` in Neovim", see `chrome-ext/README.md`): compiles `chrome-ext/src/*.ts` (two `tsc` passes), rebuilds `web/dist` + grid-renderer, and lays out a load-ready dir with the engine + one runtime variant. Load via chrome://extensions → Load unpacked. | Needs `build-nvim.sh` + npm deps in `wasm/web` and `wasm/grid-renderer`. `NVIM_EXT_VARIANT=full\|core\|minimal` (default `core`). |
...
| `node wasm/web/treesitter.test.js` | Tree-sitter in the wasm engine, both mechanisms: (1) the **bundled** grammars statically linked into `nvim.wasm` and registered via `vim._ts_add_language_builtin` (opening a `.lua` file starts highlighting; all 7 register); (2) **dynamic** grammar `.wasm` side modules dlopen'd at runtime (`-sMAIN_MODULE=2`) — by explicit path, by `runtimepath` `parser/<lang>.wasm` discovery, via the `__nvimParserFetch` host hook, and through the real worker path (`workerData.parsers` + an HTTP server). Fixture: `web/fixtures/luadyn.wasm` (lua grammar renamed `tree_sitter_luadyn`; regen with `web/fixtures/build-fixture.sh`, needs the deps configure tree). | Needs `build-nvim.sh`. Also run by `npm test` in `wasm/web` (after e2e); the worker-path scenario needs `dist/` + npm deps and skips without them. |
...
| `cd wasm/chrome-ext/e2e && go test -v` | **Headless-Chrome** e2e of the Chrome extension: loads the unpacked `_ext/` via CDP `Extensions.loadUnpacked` (branded Chrome ≥137 dropped `--load-extension`), then drives the real flow with trusted input — trigger chord on a textarea, normal-mode edit, `:w` live write-back (+ page `input` events), `:wq` teardown/focus-restore, second pre-warmed session, `:q!` no-write. | **Separate Go module** (chromedp). **Skips** without Chrome or a built `_ext/` (`build-ext.sh` first; `NVIM_EXT_DIR` to point elsewhere). |
...
| `web/src/` | browser target **TypeScript source**: `neovim.ts` (RPC core), `neovim-ui.ts` (headless Screen + canvas UI via grid-renderer), `neovim-ui-pre.ts` (simple `<pre>` renderer), `neovim-utils.ts` (helpers), `*.mts` (ESM entry points), `app.ts` (page glue), `engine-worker.ts` (Web Worker host). |
...
| `chrome-ext/` | the **Chrome extension** (edit any `<textarea>` in Neovim): `manifest.json`, `src/` (trigger content script, MV3 service worker, offscreen engine host with a pre-warmed engine pool, overlay session glue), `build-ext.sh`, `e2e/` (chromedp, separate module). See `chrome-ext/README.md`. |
