...
| `node wasm/web/treesitter.test.js` | Tree-sitter in the wasm engine: the bundled grammars are **statically linked** into `nvim.wasm` (no dlopen in wasm) and registered via `vim._ts_add_language_builtin` (treesitter.c + the EMSCRIPTEN block in src/nvim/CMakeLists.txt). Asserts opening a `.lua` file starts treesitter highlighting (parser loads, tree parses, captures resolve), all 7 bundled grammars register, and non-bundled grammars still fail cleanly. | Needs `build-nvim.sh`. Also run by `npm test` in `wasm/web` (after e2e). |
