// Package e2e is the headless-browser integration test for the tvim standalone
...
//
// It is a SEPARATE Go module (e2e/go.mod, replace tvim => ../) so chromedp's
...
//   - a built browser bundle. Either set TVIM_BUNDLE=<build-site.sh output>, or
//     have the wasm engine built (build-wasm/bin/nvim.*) so the test can run
//     wasm/web/build-site.sh itself.
//
// Run:  cd wasm/tvim/e2e && go test -v        (add -run TestBrowser… to focus)
package e2e
...
func TestBrowserProxyEndToEnd(t *testing.T) {
	...
	url := "http://" + srv.Addr() + "/"
	t.Logf("tvim server at %s (root %s)", url, root)
...
// bundleDir returns the browser bundle: TVIM_BUNDLE if set, else built via
// wasm/web/build-site.sh into a temp dir (skips if the wasm engine isn't built).
func bundleDir(t *testing.T) string {
	if b := os.Getenv("TVIM_BUNDLE"); b != "" {
		if _, err := os.Stat(filepath.Join(b, "index.html")); err != nil {
			t.Skipf("TVIM_BUNDLE=%s has no index.html: %v", b, err)
		}
		return b
	}
	_, thisFile, _, _ := runtime.Caller(0)
	repoRoot := filepath.Join(filepath.Dir(thisFile), "..", "..", "..") // e2e -> tvim -> wasm -> repo
	...
	if _, err := os.Stat(filepath.Join(repoRoot, "build-wasm", "bin", "nvim.js")); err != nil {
		t.Skip("no wasm build (build-wasm/bin/nvim.js); set TVIM_BUNDLE or run wasm/build-nvim.sh first")
	}
	out := t.TempDir()
	cmd := exec.Command("bash", buildSite, out)
	cmd.Dir = repoRoot
	if combined, err := cmd.CombinedOutput(); err != nil {
		t.Skipf("build-site.sh failed (set TVIM_BUNDLE to a prebuilt bundle): %v\n%s", err, combined)
	}
	return out
}
