...
// host's home dir (its real path — the host filesystem is mounted at the editor's
// root), so nvim loads the host's config THROUGH the proxy (proving the
// rc-from-host feature end to end). The "host" here is the in-process server; a
// fake home holds a minimal, plugin-free init.lua.
func TestBrowserRCRemote(t *testing.T) {
	...
	writeFile(t, filepath.Join(cfgDir, "init.lua"), "vim.g.tvim_rc_loaded = 'from-remote'\n")
	t.Setenv("HOME", home) // serverHome() honours $HOME (os.UserHomeDir)

	ctx := bootRCServer(t, bundle, server.Config{
		Dir: root, Port: 0,
		Assets: server.NewAssetServer(os.DirFS(bundle)), RC: "remote",
	})

	if got := evalRPC(t, ctx, `window.nvim.request('nvim_eval', ['$HOME'])`); got != home {
		t.Fatalf("$HOME = %q, want %q (rc remote should point HOME at the host home)", got, home)
	}
	if got := evalRPC(t, ctx, `window.nvim.request('nvim_eval', ['get(g:, "tvim_rc_loaded", "")'])`); got != "from-remote" {
		t.Fatalf("g:tvim_rc_loaded = %q, want from-remote (the host's init.lua did not load through the proxy)", got)
	}
}

// TestBrowserRCLocal: `--rc local` seeds the app-server's own ~/.config/nvim into
// the browser MEMFS (config dir only) and shadows $HOME/.config/nvim with it —
// $HOME being the host's home (here the app-server's own, the degenerate
// same-machine case).
func TestBrowserRCLocal(t *testing.T) {
	...
	appHome := t.TempDir() // the app-server's own home (== the IO host's home here)
	cfgDir := filepath.Join(appHome, ".config", "nvim")
	if err := os.MkdirAll(cfgDir, 0o755); err != nil {
		t.Fatal(err)
	}
	writeFile(t, filepath.Join(cfgDir, "init.lua"), "vim.g.tvim_rc_loaded = 'from-local'\n")
	t.Setenv("HOME", appHome)

	ctx := bootRCServer(t, bundle, server.Config{
		Dir: t.TempDir(), Port: 0,
		Assets: server.NewAssetServer(os.DirFS(bundle)), RC: "local",
	})

	// $HOME is the host's home (the seed shadows $HOME/.config/nvim over it).
	if got := evalRPC(t, ctx, `window.nvim.request('nvim_eval', ['$HOME'])`); got != appHome {
		t.Fatalf("$HOME = %q, want %q (the host home)", got, appHome)
	}
	if got := evalRPC(t, ctx, `window.nvim.request('nvim_eval', ['get(g:, "tvim_rc_loaded", "")'])`); got != "from-local" {
		t.Fatalf("g:tvim_rc_loaded = %q, want from-local (the seeded config did not load)", got)
	}
}
...
func TestBrowserRCLocalHomeMapped(t *testing.T) {
	...
	// The REMOTE side: the remote user's home with the remote's OWN nvim config,
	// and a marker file that exists only on the remote.
	...
	ctx := bootRCServer(t, bundle, server.Config{
		Port:   0,
		Assets: server.NewAssetServer(os.DirFS(bundle)), RC: "local",
		// The remote subprocess runs with HOME=remoteHome via `env`, so it reports
		// that home regardless of the inherited app-server HOME.
		RemoteCommand: []string{"env", "HOME=" + remoteHome, bin, "--serve-stdio"},
	})

	if got := evalRPC(t, ctx, `window.nvim.request('nvim_eval', ['$HOME'])`); got != remoteHome {
		t.Fatalf("$HOME = %q, want %q (the remote home)", got, remoteHome)
	}
	...
	if got := evalRPC(t, ctx, `window.nvim.request('nvim_eval', ['join(readfile("`+remoteHome+`/marker.txt"), "")'])`); !strings.Contains(got, "on the remote") {
