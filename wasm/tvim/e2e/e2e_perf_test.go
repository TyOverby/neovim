...
func TestBrowserPerfTrack(t *testing.T) {
	...
	t.Run("fs request/response is a custom-track measure", func(t *testing.T) {
		...
		if dt.DataType != "track-entry" || dt.Track != "IO proxy" || dt.TrackGroup != "tvim" {
			t.Errorf("devtools payload = %+v, want dataType=track-entry track=%q trackGroup=tvim", dt, "IO proxy")
		}
		if dt.Color != "primary" {
