...
It is a **separate Go module** (`go.mod`, `replace tvim => ../`) on purpose: it
pulls in chromedp's large dependency tree, which must never touch the lean
production `tvim` module. This module is dev/CI tooling, not shipped.
...
TVIM_BUNDLE=/path/to/build-site-output go test -v
```

Prerequisites (the test **skips**, not fails, when missing):
- a Chrome/Chromium binary on PATH (Chrome ≥ 137 for JSPI — verified on 149);
- a built browser bundle: either `TVIM_BUNDLE=<build-site.sh output>`, or the wasm
