...
# via env.
#
#   ./download-tvim.sh                 # host os/arch  -> ./tvim
...
# though every build-tvim job succeeded and uploaded fresh artifacts. Filtering
...
tmpdir="$(mktemp -d "${OUT%/}/.tvim-dl.XXXXXX")"
...
[ -n "$run_id" ] || { echo "error: no recent run has artifact ${ARTIFACT} (expired, or the build-tvim jobs failed?)" >&2; exit 1; }
echo "==> Downloaded ${ARTIFACT} from run ${run_id}"

# The artifact extracts to a file named after the os/arch; land it at ./tvim.
src="${tmpdir}/${ARTIFACT}"
bin="${OUT%/}/tvim"
