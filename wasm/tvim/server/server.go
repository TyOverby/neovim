...
type ConnConfig struct {
	// Dir is the io-proxy's working directory — the project dir tvim was started
...
type Config struct {
	...
	// `ssh -T <host> tvim --serve-stdio` (the three-tier architecture); tests use
	// a local `tvim --serve-stdio` subprocess.
	...
	// daemon's unix socket; SelfExe is this tvim binary (to auto-spawn the daemon).
...
func (s *Server) handleProxyConfig(w http.ResponseWriter, r *http.Request) {
	...
	nvimSock := filepath.Join(tmpBase, fmt.Sprintf("tvim-nvim-%d-%d.sock", os.Getpid(), atomic.AddInt64(&nvimSockSeq, 1)))
	cfg := map[string]any{
		"url":        "ws://" + host + "/proxy",
		"nvimSocket": nvimSock,
	...
	fmt.Fprintf(w, "// Generated by tvim: visiting this server == the standalone neovim.js app.\n"+
...
func (s *Server) serveConn(cn *conn, session string) {
	...
	if session != "" {
		...
		if d, err := connectDaemon(sock, selfExe, session); err != nil {
			log.Printf("tvim: session-host attach failed (%v); PTYs are non-durable this session", err)
		} else {
			ctx.daemon = d
			go d.pump(cn)
			defer d.close()
...
func (s *Server) dispatch(cn *conn, ctx *Ctx, h proxy.Header, payload []byte) {
	...
	case proxy.THello:
		...
		if h.Version != 0 && h.Version != proxy.ProtocolVersion {
			log.Printf("tvim: proxy protocol version mismatch: client=%d server=%d", h.Version, proxy.ProtocolVersion)
		}
