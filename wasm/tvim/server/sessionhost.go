...
// SESSION HOST (stage 5 — durable PTYs). The `tvim --session-host` daemon makes
...
// SessionHost is the `tvim --session-host` daemon: a table of durable PTY
...
// $XDG_RUNTIME_DIR/tvim (the standard per-user runtime dir, tmpfs + 0700), with a
// /tmp/tvim-<uid> fallback. The same function runs on both sides (the daemon binds
// it, the io-proxy dials it), so they always agree.
func DefaultDaemonSock() string {
	dir := os.Getenv("XDG_RUNTIME_DIR")
	if dir != "" {
		dir = filepath.Join(dir, "tvim")
	} else {
		dir = filepath.Join(os.TempDir(), fmt.Sprintf("tvim-%d", os.Getuid()))
	}
	return filepath.Join(dir, "host.sock")
}

// RunSessionHost is the `tvim --session-host` entrypoint: a SINGLETON daemon. It
