// wasm/web/src/neovim.mts - ESM entry point for the headless msgpack-RPC core.
//
// WHY THIS SHAPE: the source of truth is the core module (src/neovim.ts), which
// the build emits as the UMD `neovim.js`. The UMD file must stay usable as a
// <script> global, via require() in Node, and via importScripts() in the engine
// worker -- none of which we may break. Rather than fork the implementation,
// this ESM entry imports neovim.js purely for its SIDE EFFECT of populating a
// global, then re-exports the same API as proper ESM named + default exports.
//
// This works in a real browser with no extra wiring: when neovim.js runs in ESM
...
// global. We read it back off `globalThis` here. In Node ESM the UMD
// `module.exports` branch is taken instead and the global is not set, so we fall
// back to a dynamic `import()` of the CommonJS module via its default export.
// Either way the named surface below is identical to the UMD object.
import './neovim.js';
import type {
...
const ns: any =
  (typeof globalThis !== 'undefined' && (globalThis as any).Neovim) ||
  ((await import('./neovim.js')) as any).default ||
  {};
...
let bundledMP: any = null;
try {
  // @ts-ignore - copied next to the bundle by build-lib.sh; absent in the source tree.
  bundledMP = await import('./msgpack.esm/index.mjs');
} catch (_e) {
  bundledMP = null;   // not bundled (e.g. running from the source tree) -- fine.
}

function withMessagePack<T extends { MessagePack?: MessagePackModule }>(opts?: T): T {
  opts = (opts || {}) as T;
  if (opts.MessagePack) { return opts; }   // caller override wins; don't clobber.
  const mp = (typeof globalThis !== 'undefined' && (globalThis as any).MessagePack) || bundledMP;
  if (!mp) { return opts; }                // none available: let createNvim error clearly.
  return Object.assign({}, opts, { MessagePack: mp });
}

export function create(opts?: CreateOptions): NeovimFacade { return ns.create(withMessagePack(opts)); }
export function createNvim(opts?: CreateNvimOptions): NeovimInstance { return ns.createNvim(withMessagePack(opts)); }
...
// Re-export the public types so `import { create } from 'neovim'` consumers also
