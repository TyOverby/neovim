// wasm/web/src/neovim-ui-pre-testutil.ts - TESTING UTILITY: the legacy
// "render into a <pre>" DOM renderer.
//
// This was the original default UI renderer before the canvas renderer
// (neovim-ui.ts + wasm/grid-renderer) replaced it on the page. It is kept
// as a testing/debugging utility:
//
//   * render(el, screen) turns a headless Screen into styled HTML - handy for
//     asserting on / eyeballing decoded screen state without a canvas stack
//     (tests can inspect el.innerHTML as a plain string).
//   * mount_into(instance, el) is the complete old <pre> UI, still functional
//     for A/B comparison against the canvas renderer.
//
// It is NOT UMD-wrapped and NOT shipped by build-site.sh / build-lib.sh; the
// build emits it as a plain CommonJS module in dist/ for Node tests to
// require(). The headless Screen model itself lives in neovim-ui.ts (shared
// with the canvas renderer) and is imported from there.
...
// ---- mount_into ---------------------------------------------------------
// The legacy <pre> mount: wire `instance` to render into the DOM element `el`
// (a <pre>) and forward its keystrokes. Semantics identical to the old
// default renderer; see the header comment for why this is a test utility.
